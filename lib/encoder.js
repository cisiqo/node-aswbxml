...
function parseTag(obj, tagName, codePage, codePageMapping, currentCodePage) {
	var tagMatch = tagName.match(/^(?:(.+)\:)?(.+)$/),
		...
		hasContent = (obj !== '') && ((proto === '[object Object]' && Object.keys(obj).length !== 0 && Object.keys(obj) !== ['$']) ||
					(proto === '[object String]'));
	...
	tagBuffer = Buffer.concat([ tagBuffer, new Buffer([tagCode.token | (hasContent? 64 : 0)]) ]);
	//TODO: add attributes
	...
	if (hasContent) {
		if (proto === '[object String]') {
			...
			Object.keys(obj).forEach(function(key) {
				...
				if (key === 'cdata') {
					tagBuffer = Buffer.concat([ 
						tagBuffer,  
						new Buffer(['0xC3']), 
						encode_mb_u_int32(obj[key].length), 
						new Buffer(obj[key])
					]);
				} else {
		...
		return {
			buffer: Buffer.concat([ tagBuffer ]),
			codePage: currentCodePage
		};
	}
...
function encode_mb_u_int32(val) {
	var out=[], b;
	do {
		b = val & 127;
		if (out.length > 0) {
